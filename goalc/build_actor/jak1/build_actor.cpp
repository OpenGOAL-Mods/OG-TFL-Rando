...
size_t generate_dummy_merc_ctrl(DataObjectGenerator& gen, const ArtGroup& ag) {
  ...
  auto effect_count = ag.model->effects.size();
  gen.add_word(0);                                   // 4
  ...
  gen.add_word(ag.model->effects.size());            // 56 (effect-count)
  ...
  generate_merc_effects(gen, ag.model, joints);
  return result;
}
...
bool run_build_actor(const std::string& mdl_name,
  ...
  ag.model = &extract_data.new_model;
  // MercSwapData out;
